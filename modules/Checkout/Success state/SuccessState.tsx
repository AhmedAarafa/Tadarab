        ...
        if (invoiceDetails) {
            ...
            if (checkoutType == "cart") {
                tadarabGA.tadarab_fire_traking_GA_code("purchase", {
                    id: invoiceDetails?.data?.data?.transaction_details?.invoice_no,
                    revenue: invoiceDetails?.data?.data?.transaction_details?.amount_usd,
                    coupon: invoiceDetails?.data?.data?.transaction_details?.coupon,
                    products: invoiceDetails?.data?.data?.transaction_details?.transaction_items,
                    uid: invoiceDetails?.data?.data?.ga_tracking?.uid,
                    cid: invoiceDetails?.data?.data?.ga_tracking?.cid,
                    email: invoiceDetails?.data?.data?.transaction_details?.email,
                });

                let customData = { value: invoiceDetails?.data?.data?.transaction_details?.amount_usd, currency: 'USD', content_type: 'online_course_purchase' };
                FBPixelEventsHandler(invoiceDetails?.data?.fb_tracking_events, customData);
                ...
                tadarabGA.tadarab_fire_traking_GA_code("subscription", {
                    ...
                    user_id: invoiceDetails?.data?.data?.transaction_details?.user_id,
                    revenue: invoiceDetails?.data?.data?.transaction_details?.amount_usd,
                    date: invoiceDetails?.data?.data?.transaction_details?.date_ymd,
                    cid: invoiceDetails?.data?.data?.ga_tracking?.cid,
                    is_trial_free: invoiceDetails?.data?.data?.transaction_details?.is_trial_free,
                    free_trial_label: invoiceDetails?.data?.data?.transaction_details?.free_trial_label,
                    subscription_elabel: invoiceDetails?.data?.data?.transaction_details?.subscription_elabel,
                ...
                <Col sm={6} xs={12}>
                    <div className={styles["success-state-row__invoice-box"]}>
                        ...
                        <div className={styles["success-state-row__invoice-box__details-box"]}>
                            ...
                            {invoiceDetails?.data?.data?.transaction_details?.knet_transaction_id && <div className={styles["success-state-row__invoice-box__details-box__details"]}>
                                <div> رقم عملية الشراء </div>
                                <div> {invoiceDetails?.data?.data?.transaction_details.knet_transaction_id} </div>
                            </div>}


                            {invoiceDetails?.data?.data?.transaction_details?.bank_reference && <div className={styles["success-state-row__invoice-box__details-box__details"]}>
                                <div> الرقم المرجعي </div>
                                <div> {invoiceDetails?.data?.data?.transaction_details.bank_reference} </div>
                            </div>}
